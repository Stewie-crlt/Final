  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::rm_msgs::MapSentryData_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.delta_x.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int8_t>::stream(s, true ? std::string() : indent + "    ", v.delta_x[i]);
    }
    if (v.delta_x.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.delta_y.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int8_t>::stream(s, true ? std::string() : indent + "    ", v.delta_y[i]);
    }
    if (v.delta_y.empty() || true)
      s << "]";
    if (true || !indent.empty())
